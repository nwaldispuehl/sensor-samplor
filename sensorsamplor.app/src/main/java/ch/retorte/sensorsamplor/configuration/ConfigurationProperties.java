package ch.retorte.sensorsamplor.configuration;

/**
 * A list of known configuration properties.
 */
public class ConfigurationProperties {

  public static final String SENSOR_PLATFORM_IDENTIFIER = "sensorsamplor.sensor_platform_identifier";

  public static final String ACTIVE_SENSORS = "sensorsamplor.active_sensors";

  public static final String ACTIVE_RECEIVERS = "sensorsamplor.active_receivers";

  public static final String RECEIVER_SENSOR_TYPE_PATTERN = "sensorsamplor.receiver_sensor_type_pattern";

  public static final String RECEIVER_PLATFORM_IDENTIFIER_PATTERN = "sensorsamplor.receiver_platform_identifier_pattern";

  public static final String MEASUREMENT_CRON_EXPRESSION = "sensorsamplor.measurement_cron_expression";

  public static final String BUS_NAME = "sensorsamplor.bus.name";

  public static final String BUS_USERNAME = "sensorsamplor.bus.username";

  public static final String BUS_PASSWORD = "sensorsamplor.bus.password";

  public static final String INTERFACES = "sensorsamplor.bus.interfaces";

  public static final String BUFFER_SIZE = "sensorsamplor.bus.buffer_size";

}
